...
export async function recalculatePeriods() {
  try {
    ...
    for (const period of existingPeriods) {
      ...
      console.log(`Dönem silindi: ${period.id}`);
    }
    ...
    financialData.forEach((financial, index) => {
      console.log(`Finansal kayıt ${index + 1}:`, financial);
      ...
      if (financial.type === "income") {
        ...
        console.log(`${periodKey} dönemine ${amount} ${currency} gelir eklendi (${amountInTRY} TRY)`);
        ...
        console.log(`${periodKey} dönemine ${amount} ${currency} şirket gideri eklendi (${amountInTRY} TRY)`);
      }
    });
    ...
    console.log(`${Object.keys(processedPeriods).length} adet dönem kaydı eklenecek`);
    ...
    for (const key in processedPeriods) {
      ...
      try {
        ...
        console.log(`Dönem eklendi: ${periodData.year}-${periodData.month}, TRY Gelir: ${periodData.totalIncomeInTRY}₺, Müşteri: ${periodData.customerCount}`);
      } catch (error) {
        console.error(`Dönem eklenirken hata: ${periodData.id}`, error);
      }
    }
    
    console.log(`Dönem hesaplaması tamamlandı.`);
